...
def enforce_output_dimensions(image_bytes: bytes, recipient_size: Tuple[int, int]) -> bytes:
    """Match recipient dimensions without distorting aspect ratio (center-pad if needed)."""
...
def align_donor_to_recipient(
    ...
    scaled_size = (
        max(1, int(round(donor_rgb.size[0] * scale))),
        max(1, int(round(donor_rgb.size[1] * scale))),
    )
    donor_scaled = donor_rgb.resize(scaled_size, Image.LANCZOS)
    donor_bbox_scaled = (
        int(round(donor_bbox[0] * scale)),
        int(round(donor_bbox[1] * scale)),
        int(round(donor_bbox[2] * scale)),
        int(round(donor_bbox[3] * scale)),
    )
...
def apply_shadow_mask_to_recipient(
    ...
    # Model mask: white=no shadow, black=full shadow.
    ...
    if _env_flag("SHADOW_MASK_ALIGN") is not False:
    ...
    return output_rgba, preview_rgba
...
def ensure_low_res_png(src_path: Path, role: str, long_edge: int) -> Path:
    ...
    if low_dir:
        ...
        if not low_path.exists():
            ...
            convert_to_low_res_png(src_path, low_path, long_edge)
        else:
    ...
    if not fallback.exists():
        ...
        convert_to_low_res_png(src_path, fallback, long_edge)
    else:
...
def request_shadowed_image(
    ...
    recipient_mime, recipient_bytes, recipient_size = load_image_bytes_for_api(
        recipient_path, max_side
    )
    donor_mime, donor_bytes, _ = load_image_bytes_for_api(donor_path, max_side)
    prompt = prompt_text
    print(
        f"[submit] recipient={recipient_path.name} size={recipient_size[0]}x{recipient_size[1]} "
    ...
    for attempt in range(1, max(1, attempts) + 1):
        try:
            ...
            final_bytes = enforce_output_dimensions(raw_bytes, recipient_size)
            if should_flatten_output():
...
def main() -> None:
    ...
    shadow_only_mode = is_shadow_only(prompt_path)
    ...
    def process_one(item: dict) -> None:
        ...
        try:
            raw_bytes, image_bytes = request_shadowed_image(
                client_local, model_id, recipient_low, donor_low, effective_max_side, prompt_text
            )
        except Exception as exc:  # noqa: BLE001
        ...
        if shadow_only_mode:
            # Treat model output as grayscale shadow mask.
            out_rgba, preview_rgba = apply_shadow_mask_to_recipient(recipient_low, image_bytes)
            ...
            with Image.open(BytesIO(image_bytes)) as mask_img:
